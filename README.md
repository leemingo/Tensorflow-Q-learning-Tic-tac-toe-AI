...
```
1. Q(S,A) = Q(S,A) + learning_rate * [R + discount_factor * Max(Q(S',a)) - Q(S,A)]
2. Y = R + discount_factor * Max(Q(S',a))
3. Loss(W) = MSE(Qw(S,A) - Y)
4. Gradient descent : W = W - learning_rate * gradient(Loss(W))
```
...
* DP-method나 Table을 이용한 Q-learning에 비해 느리게 학습이 됩니다.
* 모든 state를 Table에 저장할 수 있는 Tictactoe게임에서 효율적인 방법은 아닙니다.
